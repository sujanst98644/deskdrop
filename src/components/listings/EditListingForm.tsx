...
export function EditListingForm({ listing, categories }: EditListingFormProps) {
  ...
  const {
    ...
    defaultValues: {
      ...
      condition: listing.condition as ListingInput["condition"],
      categoryId: listing.categoryId,
      campusCity: listing.campusCity || "",
  ...
  const onSubmit = (data: ListingInput) => {
    ...
    startTransition(async () => {
      const result = await updateListingAction(null, formData);

      if (!result.success && result.errors) {
        setServerErrors(result.errors);
        Object.entries(result.errors).forEach(([key, messages]) => {
          if (key !== "_form") {
            setError(key as keyof ListingFormValues, { message: messages[0] });
          }
        });
        ...
        setSuccessMessage("Listing updated! Redirecting...");
        setTimeout(() => {
